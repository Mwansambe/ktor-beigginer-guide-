...
fun Application.module() {
    ...
    routing {
        route("/habits") {
            ...
            get {
                call.respond(HttpStatusCode.OK, habitList)
            }
            ...
            // UPDATE (MODULE 04 EXERCISE SOLUTION)
            put("/{id}") {
                val id = call.parameters["id"]?.toIntOrNull() ?: throw IllegalArgumentException("Invalid ID")
                ...
                val savedHabit = habitList[index].copy(
            ...
            delete("/{id}") {
                val id = call.parameters["id"]?.toIntOrNull() ?: throw IllegalArgumentException("Invalid ID")
                val removed = habitList.removeIf { it.id == id }
                if (removed) {
                    call.respondText("Habit $id deleted successfully", status = HttpStatusCode.OK)
                } else {
